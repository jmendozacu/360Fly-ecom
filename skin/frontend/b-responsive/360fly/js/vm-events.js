...
jQuery(document).ready(function($){
    ...
    function visible(){
        var visibleClass;
        if($(".hide-hd-specs").css('display') == 'none'){
            visibleClass = ".4k";
        }else if($(".hide-4k-specs").css('display') == 'none'){
            visibleClass = ".hd";
        }
        return visibleClass;
    }
    ...
    if ($(window).width() < 767){
        var checkVisible = visible();
            $(checkVisible+" .keyPoints").each(function( index ) {
                cHTML = $(this).html();
                $(checkVisible+" ul.mobile").append(cHTML);
            });

            $(checkVisible+" ul.mobile div").each(function( index ) {
                cHTML = $(this).html();
                $(checkVisible+" ul.mobile").append("<li>" + cHTML + "</li>");
                $(this).remove();
            });
