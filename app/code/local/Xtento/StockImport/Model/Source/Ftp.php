 ...
 * Last Modified: 2016-03-21T15:39:41+01:00
...
class Xtento_StockImport_Model_Source_Ftp extends Xtento_StockImport_Model_Source_Abstract
{
    ...
    public function loadFiles()
    {
        ...
        /* Alternate version to get file list for FTP servers that use incompatible ftp_nlist results
        ...
        */
        ...
        if (Mage::registry('stock_import_profile')->getId() == 2) {
            $filelist = ftp_rawlist($this->_connection, "");
            $results = array();
            foreach ($filelist as $line) {
                list($perms, $links, $user, $group, $size, $d1, $d2, $d3, $name) = preg_split('/\s+/', $line, 9);
                if ($name == '.' || $name == '..') continue;
            ...
            $filelist = array(array_pop($results));
            if (isset($filelist[0])) {
                $filelist = array($filelist[0]["name"]);
            } else {
                return false;
            }
        } else {
            ...
            if ($forceDelete) {
                ...
                if ($chDir) {
                    if (!@ftp_chdir($this->_connection, $this->getSource()->getArchivePath())) {
                        $logEntry->setResult(Xtento_StockImport_Model_Log::RESULT_WARNING);
                        $logEntry->addResultMessage(Mage::helper('xtento_stockimport')->__('Source "%s" (ID: %s): Could not change directory on FTP server to archive directory. Please make sure the directory exists and that we have rights to read/write in the directory. Also, make sure the archive path is an absolute path, i.e. that it begins with a slash (/).', $this->getSource()->getName(), $this->getSource()->getId()));
                        return false;
                    }
                }
                foreach ($filesToProcess as $file) {
