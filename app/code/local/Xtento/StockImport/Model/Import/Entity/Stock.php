 ...
 * Last Modified: 2016-04-14T15:25:46+02:00
...
class Xtento_StockImport_Model_Import_Entity_Stock extends Xtento_StockImport_Model_Import_Entity_Abstract
{
    ...
    {
        Mage::dispatchEvent('xtento_stockimport_stockupdate_before', array(
        ...
        if (!in_array('qty', $fieldsFound) && !in_array('is_in_stock', $fieldsFound) && !in_array('backorders', $fieldsFound) && !in_array('manage_stock', $fieldsFound) && !in_array('notify_stock_qty', $fieldsFound)) {
    ...
    private function _getCurrentStockInfo()
    {
        ...
        $select = $this->_readAdapter->select()
            ->from($this->getTableName('cataloginventory/stock_item'), array('product_id', 'qty', 'is_in_stock', 'stock_id', 'manage_stock', 'notify_stock_qty', 'use_config_manage_stock', 'min_qty', 'use_config_min_qty', 'backorders', 'use_config_backorders'))
        ...
        foreach ($updatedFields as $field => $value) {
            if ($field == 'qty' && !isset($updateData['is_in_stock'])) {
                ...
                if (!isset($stockItem) || (int)$stockItem['use_config_backorders'] === 1) {
                    $allowBackorders = (int)Mage::getStoreConfig('cataloginventory/item_options/backorders');
                } else {
    ...
    public function afterRun()
    {
        ...
        try {
            if (!empty($this->_modifiedStockItems)) {
                ...
                foreach ($configurableProducts as $configurableProduct) {
                    ...
                    $stockItem = Mage::getModel('cataloginventory/stock_item')->loadByProduct($configurableProduct);
                    if ($isInStock === false) {
            ...
            if ($this->getConfigFlag('update_low_stock_date')) {
                ...
                if (Mage::helper('xtcore/utils')->isExtensionInstalled('Innoexts_Warehouse')) {
                    // Innoexts_Warehouse is installed, refresh for all stock_ids
                    $stockIds = $this->_readAdapter->fetchCol('SELECT stock_id FROM '.$this->getTableName('cataloginventory/stock').' GROUP BY stock_id');
                    foreach ($stockIds as $stockId) {
                        Mage::getResourceSingleton('cataloginventory/stock')->setStockId($stockId)->updateLowStockDate();
                    }
                } else {
        ...
        Mage::dispatchEvent('xtento_stockimport_stockupdate_after', array(
            'profile' => $this->getProfile(),
            'log' => $this->getLogEntry(),
            'modified_stock_items' => $this->_modifiedStockItems
        ));
